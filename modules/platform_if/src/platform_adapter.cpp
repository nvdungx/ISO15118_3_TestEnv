...
namespace TestLib
{
const uint32_t GENERATOR8 = 0x1D;
const uint32_t GENERATOR16 = 0x1021;
...
// 0x04C11DB7: this because 1st MSB = 1 always shift out
const uint32_t GENERATOR32 = 0x04C11DB7;
...
uint32_t get_crc32_tb(const std::vector<uint8_t> &dataArr)
{
  ...
  for (auto &data : dataArr)
  {
    ...
    pos = (uint8_t)(data ^ (crc >> 24));
    /* Shift out the MSB used for division per lookuptable and XOR with the remainder */
    crc = ((crc << 8) ^ (CRC_TABLE32[pos]));
  }
  return crc;
}

void cal_crcTable32(void)
{
  uint32_t curByte = 0;
  ...
  for (int divident = 0; divident < 256; divident++)
  {
    ...
    curByte = (uint32_t)(divident << 24);
    for (uint8_t bit = 0; bit < 8; bit++)
    {
      if ((curByte & 0x80000000) != 0)
      {
        curByte = (curByte << 1) ^ GENERATOR32;
      }
      else
      {
        curByte <<= 1;
