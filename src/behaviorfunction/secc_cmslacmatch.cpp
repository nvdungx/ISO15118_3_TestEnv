  ...
  while (v_repetition)
  {
    ...
    while (true)
    {
      ...
      if (SECC_Tester::tc_TT_match_response->timeout())
      {
        ...
        {
          SLOGI("The repetition limit is not reached, a new CM_SLAC_MATCH.REQ message will be send.");
        }
      }
...
VerdictValue f_SECC_CMN_TB_VTB_CmSlacMatch_002(void)
{
  ...
  SECC_Tester::pt_SLAC_Port->send(md_CMN_CMN_SlacMme_001(
      md_CMN_CMN_SlacMmeCmnHeader_001({CM_SLAC_MATCH_REQ = "607C"}),
      md_CMN_CMN_CmSlacMatchReq_001(m_CMN_CMN_SlacPayloadHeader_001(), par_testSystem_mac, vc_sut_mac, vc_RunID))); // to vc_sut_mac;
  while (true)
  {
    if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
            md_CMN_CMN_SlacMmeCmnHeader_001({CM_SLAC_MATCH_CNF = "607D"}),
            md_CMN_CMN_CmSlacMatchCnf_001(m_CMN_CMN_SlacPayloadHeader_001(), par_testSystem_mac, vc_sut_mac, vc_RunID, MATCH_ANY, MATCH_ANY))))
    {
      // repeat;
    }
    if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
...
VerdictValue f_SECC_CMN_TB_VTB_CmSlacMatch_003(void)
{
  ...
  else
  {
    ...
    SECC_Tester::tc_TT_EVSE_match_session->start(par_TT_EVSE_match_session + par_CMN_Transmission_Delay);
  ...
  while (true)
  {
    if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
            ...
            md_CMN_CMN_CmSlacMatchCnf_001(m_CMN_CMN_SlacPayloadHeader_001(), par_testSystem_mac, vc_sut_mac, vc_RunID, MATCH_ANY, MATCH_ANY))))
    {
      setverdict(fail, "CM_SLAC_MATCH.CNF was not expected, " & v_timer_name & " timer has been expired.");
      SECC_Tester::tc_TT_match_response->stop();
    }
...
VerdictValue f_SECC_CMN_TB_VTB_CmSlacMatch_004(const MME_Payload &v_payload)
{
  ...
  SECC_Tester::pt_SLAC_Port->send(md_CMN_CMN_SlacMme_001(
      md_CMN_CMN_SlacMmeCmnHeader_001({CM_SLAC_MATCH_REQ = "607C"}), v_payload)); // to vc_sut_mac;
  while (true)
  {
    if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
      ...
      while (true)
      {
        if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
                ...
                md_CMN_CMN_CmSlacMatchCnf_001(
                    ...
                    par_testSystem_mac, vc_sut_mac, vc_RunID, MATCH_ANY, MATCH_ANY))))
        {
          setverdict(fail, "CM_SLAC_MATCH.CNF was not expected, " & v_timer_name & " has been expired.");
          SECC_Tester::tc_TT_match_response->stop();
        }
        if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
                md_CMN_CMN_SlacMmeCmnHeader_001({CM_ATTEN_CHAR_IND = "606E"}), MATCH_ANY)))
...
VerdictValue f_SECC_CMN_TB_VTB_CmSlacMatch_005(const std::shared_ptr<HAL_61851_Listener> &v_HAL_61851_Listener)
{
  ...
  while (true)
  {
    if (SECC_Tester::pt_SLAC_Port->receive(md_CMN_CMN_SlacMme_001(
            md_CMN_CMN_SlacMmeCmnHeader_001({CM_SLAC_MATCH_CNF = "607D"}),
            md_CMN_CMN_CmSlacMatchCnf_001(
                m_CMN_CMN_SlacPayloadHeader_001(), par_testSystem_mac, vc_sut_mac, vc_RunID, MATCH_ANY, MATCH_ANY))))
    {
