...
VerdictValue f_EVCC_CMN_TB_VTB_PLCLinkStatus_001(VerdictValue v_vct)
{
  ...
  EVCC_Tester::pt_SLAC_Port->send(md_CMN_CMN_SlacMme_001(
      ...
      md_CMN_CMN_CmSlacMatchCnf_001(m_CMN_CMN_SlacPayloadHeader_001(), vc_sut_mac, par_testSystem_mac, vc_RunID, vc_Nid, vc_Nmk))); // to vc_sut_mac;
...
VerdictValue f_EVCC_CMN_TB_VTB_PLCLinkStatus_002()
{
  ...
  v_verdict = f_EVCC_getPLCLinkTermination(par_TP_match_leave + par_CMN_Transmission_Delay, fail);
  if (v_verdict == pass)
  {
...
VerdictValue f_EVCC_CMN_TB_VTB_PLCLinkStatus_005(HAL_61851_Listener v_HAL_61851_Listener, float v_time)
{
  ...
  timer statetimer = (PICS_CMN_CMN_WakeUp - v_time + 5.0);
  statetimer.start();
  while (true)
  {
...
VerdictValue f_EVCC_CMN_TB_VTB_PLCLinkStatus_006(HAL_61851_Listener v_HAL_61851_Listener, integer v_dutyCycle, IEC_61851_States v_state)
{
  ...
  if (v_verdict == pass)
  {
    ...
    v_verdict = f_EVCC_confirmState(valid, v_HAL_61851_Listener, statetimer, fail);
  }
  if (v_verdict == pass)
  {
...
VerdictValue f_EVCC_CMN_TB_VTB_PLCLinkStatus_007()
{
  ...
  EVCC_Tester::pt_SLAC_Port->send(md_CMN_CMN_SlacMme_001(md_CMN_CMN_SlacMmeCmnHeader_001({CM_NW_STATS_REQ = "6048"}),
